...
`kernel_config` node provides kernel configuration:
  ...
  * `share_default_cmdline` allows default cmdline to be shared with the cmdline of the specified kernel pkgbase under `kernel_config` node. The default cmdline `cmdline`, `fallback_cmdline` or `/etc/kernel/cmdline`, `/etc/kernel/fallback-cmdline` files will be used as a shared cmdline for all kernels. This option only takes effect if the `cmdline_per_kernel` option is enabled. By default this option is set to `false`
  ...
  * `pkgbase` (kernel package name) node provides additional configuration for specified kernel package name:
